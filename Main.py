...
def Quiz(Timed):
    ...
    def Answer(Response,Definition):
        ...
        if len(Quiz_Arr) != 0:
            ...
            if Timed:
                ...
                Try_Again = CTkMessagebox(title="Try_Again?", message=f"You failed {Fails} many times and you took {Time_Taken} seconds long,Would you like to try again?",icon="question", option_1="No", option_2="Yes")
            else:
                pass
                Try_Again = CTkMessagebox(title="Try_Again?", message=f"You failed {Fails} many times.Would you like to try again?",icon="question", option_1="No", option_2="Yes")
    ...
    def Pause():
        ...
        Pause_M = CTkMessagebox(title="Paused", message="Continue quiz or leave?",icon="question", option_1="Leave", option_2="Continue")
    ...
    Answer_A_B = CTkButton(Quiz_W, command = lambda : Answer(Definition_A,Definition), text = "A",width = 28)
    Answer_B_B = CTkButton(Quiz_W, command = lambda : Answer(Definition_B,Definition), text = "B",width = 28)
    Answer_C_B = CTkButton(Quiz_W, command = lambda : Answer(Definition_C,Definition), text = "C",width = 28)
...
def Options(): 
    ...
    Add_B = CTkButton(Options_W, command = lambda : Add(Add_File_Arr), text = "Add Contents")
    Create_B = CTkButton(Options_W, command = lambda : Create(Add_File_Arr), text = "Create File")
